...
from molcrystalflow.data.dataset import ATOM_TYPE_TO_IDX, get_equivariant_axes
...
def run_inference(
    ...
    for flip_state, n_samples in flip_states:
        ...
        # Keep legacy CLI knobs working with the current model API.
        if hasattr(model, "interpolant"):
            if hasattr(model.interpolant, "_trans_cfg"):
                model.interpolant._trans_cfg.scaling = scaling
            if hasattr(model.interpolant, "_rots_cfg"):
                model.interpolant._rots_cfg.exp_rate = exp_rate

        # Run inference
        with torch.inference_mode():
            results = model(batch, num_timesteps=num_timesteps)
