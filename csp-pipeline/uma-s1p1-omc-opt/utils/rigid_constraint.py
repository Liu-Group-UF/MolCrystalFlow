...
def add_rigid_body_constraints(atoms):
    """Add rigid body constraints based on building block indices.
    
    Supports both 'bb_indices' and 'bb_idx' array names for compatibility.
    """
    # Support both bb_indices and bb_idx for compatibility
    if 'bb_indices' in atoms.arrays:
        bb_idx = atoms.arrays['bb_indices']
    elif 'bb_idx' in atoms.arrays:
        bb_idx = atoms.arrays['bb_idx']
    else:
        raise ValueError("Atoms object must have 'bb_indices' or 'bb_idx' array")
