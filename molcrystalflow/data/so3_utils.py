...
def vec2carat(w):
    """w [...,3] -> skew-symmetric [...,3,3]"""
    wx, wy, wz = w.unbind(-1)
    O = torch.zeros_like(wx)
    return torch.stack([
        torch.stack([ O,   -wz,  wy], -1),
        torch.stack([ wz,   O,  -wx], -1),
        torch.stack([-wy,  wx,   O ], -1),], -2)

# ther logmap for so3 velocity fields are by Winston
class LogMap(ABC):
...
class LogSO3(LogMap):
    def __call__(self, p, q):
        """
        Following logic on wikipedia https://en.wikipedia.org/wiki/3D_rotation_group#Logarithm_map
        as well as MOFFlow paper
        ...
        """
        R = torch.einsum('bkj,bji->bki',p.transpose(-1,-2),q)
        skewSym = (R - R.transpose(-2,-1))
        
        vector = torch.zeros_like(skewSym[..., 0])
        vector[..., 0] = skewSym[..., 2, 1]
        vector[..., 1] = skewSym[..., 0, 2]
        vector[..., 2] = skewSym[..., 1, 0]
        ...
        angles_sin = torch.norm(vector, dim=-1) / 2.0
        ...
        mask_pi = torch.isclose(angles, torch.full_like(angles, torch.pi), atol=1e-2).to(angles.dtype)
        ...
        prefactor = numerator / denominator.clamp_min(1e-10)
        vector = vector * prefactor[..., None]

        # For angles close to pi, derive vectors from their outer product (ww' = 1 + R).
        id3 = torch.eye(3, device=R.device, dtype=R.dtype)
        id3 = torch.broadcast_to(id3, R.shape)
        ...
        vector_pi = torch.sqrt(torch.diagonal(skew_outer, dim1=-2, dim2=-1).clamp_min(1e-10))
        ...
        w = vector + vector_pi * mask_pi[..., None] 
        Omega = vec2carat(w)                                   # (B,3,3)
        return torch.einsum('bij,bjk->bik', p, Omega)          # (B,3,3)
